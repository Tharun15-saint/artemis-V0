"""
Backfill comparable_sales_growth_pct on Target retailer_financials from 10-Q MD&A.
...
SCRIPT_VERSION = "target-comp-sales-backfill-v1.0"
SOURCE_NAME = "target_comp_sales_backfill"
SEC_FETCH_DELAY_SECONDS = 0.15

# Pattern 1 (priority): Comparable sales change X % / (X) %
# Pattern 2 (fallback):  Comparable sales increased/decreased X percent
# Both reject values outside -30% to +40% (see _parse_comparable_sales_growth_pct_from_10q_text).
...
def run_comp_sales_backfill(db: Session, ctx: IngestionContext) -> dict[str, int]:
    ...
    if not rows:
        logger.info(
            "No Target rows with NULL comparable_sales_growth_pct and source_10q_url"
        )
        return stats

    logger.info(
        "Comparable sales backfill — %d candidate(s), newest first",
        len(rows),
    )
    ...
    for row in rows:
        ...
        comp_pct, pattern = tier1._parse_comparable_sales_growth_pct_from_10q_text(text)
        if comp_pct is None:
            stats["not_found"] += 1
            logger.info(
        ...
        if pattern == "comparable_sales_table":
            stats["updated_table"] += 1
        else:
            stats["updated_narrative"] += 1

        logger.info(
            "%s: comparable_sales_growth_pct=%s via %s",
            label,
            comp_pct,
            pattern,
        )
    ...
    logger.info(
        "Comparable sales backfill complete — candidates=%d table=%d narrative=%d "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Backfill Target comparable_sales_growth_pct from 10-Q filings"
        )
    )
    parser.add_argument(
        "--count-only",
